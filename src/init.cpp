/*
The boilerplate code used for initialisation
*/
#define OUT 
...
void Initialisation(VkInstance *Instance,std::vector<VkPhysicalDevice> *DeviceList, VkCommandPool *CmdPool) {
    //Error checking
    VkResult Result;
    //Load extensions
    ...
    vkGetPhysicalDeviceQueueFamilyProperties(DeviceList->at(0),OUT &QueueFamilyCount,NULL);
    ...
    VkDevice Device;
    Result = vkCreateDevice(DeviceList->at(0),OUT &DeviceInfo,NULL,OUT &Device);
    ...
    //TODO Make a function that makes a buffer and ties it to a specified Pool
    //create the command buffer from the command pool
    VkCommandBuffer CmdBuffer;
    VkCommandBufferAllocateInfo Cmd = {};
    CreateBuffer(&CmdBuffer,&Cmd,*CmdPool);
    ...
    for (uint32_t i = 0; i < QueueFamilyCount; i++) {
        vkGetPhysicalDeviceSurfaceSupportKHR(DeviceList->at(0), i,Surface,OUT &pSupportsPresent[i]);
    }
    ...
    // Construct the surface description:
        ...
        Result = vkGetPhysicalDeviceSurfaceCapabilitiesKHR(DeviceList->at(0),Surface,OUT &SurfCapabilities);
        assert(Result == VK_SUCCESS);

        uint32_t PresentModeCount;
        Result = vkGetPhysicalDeviceSurfacePresentModesKHR(DeviceList->at(0),Surface,OUT &PresentModeCount, NULL);
        assert(Result == VK_SUCCESS);
    ...
    vkGetPhysicalDeviceFormatProperties(DeviceList->at(0),DepthFormat,&Props);
    ...
    vkBindImageMemory(Device,Depth.image,Depth.mem,
...
void CreateBuffer(VkCommandBuffer *Buffer, VkCommandBufferAllocateInfo *Cmd, VkCommandPool CmdPool,VkCommandBufferLevel Level = VK_COMMAND_BUFFER_LEVEL_PRIMARY){
